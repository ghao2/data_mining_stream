...
def calc_para_current_chunk(current_data):
    ...
    weight_i=calc_weight_i(MSE_i, 0.50);
...
def calc_weights_k_classifiers(classifier_vector, current_data):
    ...
    for index in range(0, len(classifier_vector)):
        ...
        #here we assume MSE_r is calculated from uniform distribution, so it's value is 0.25
        weight_i=calc_weight_i(MSE_i, 0.5);
...
def calc_ensemble_classifiers(classifier_vector, current_data):
    ...
    current_data_without_label=current_data[:, 0:-1];
    ...
    return_Matrix=np.concatenate((current_data_without_label, prediction_vector), axis=1);
...
def training_by_ensembled_classifier(K, t):
    ...
    for index in range(K-1, 99):
        ...
        #this block of code if for testing purpose, produce the MSE varation matrix
        for index_predict in range(0, K):
        ...
        classified_data=calc_ensemble_classifiers(W_w0_weight_vectors_total, test_data);
        ...
        W_w0_weight_vectors=W_w0_weight_vectors_total[1:,:];
...
pl.axis([0, 10, 0, 0.5]);
pl.title('Model Incrementation=0.3, k=20');
pl.show();
#run_triditional();
